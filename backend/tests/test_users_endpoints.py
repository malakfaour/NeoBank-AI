...
async def test_get_me_returns_profile_with_unread_count(client):
    ...
    async with AsyncSessionLocal() as session:
        session.add_all(
            [
                Notification(user_id=user.id, type=NotificationType.ALERT, message="one", read=False),
                Notification(user_id=user.id, type=NotificationType.ALERT, message="two", read=False),
                Notification(user_id=user.id, type=NotificationType.ALERT, message="three", read=True),
            ]
        )
...
async def test_patch_my_email_uses_otp_verification(client, monkeypatch):
    ...
    monkeypatch.setattr("app.api.v1.endpoints.users.verify_and_consume_otp", fake_verify_and_consume_otp)
